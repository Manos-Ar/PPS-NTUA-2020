...
echo "########################################"
echo "########## Scenario 1 - TEST ###########"
echo "########################################"

## Scenario 1: For (naive, coalesced, shmem) kernels, for block-tile sizes, M = N = K = 256
for i in $gpu_kernels; do
	for b in $block_sizes; do
		make -s clean
		make -s THREAD_BLOCK_X=$b THREAD_BLOCK_Y=$b TILE_X=$b TILE_Y=$b DEBUG=0 EPS=1e-4
		GPU_KERNEL=$i $gpu_prog 256 256 256
	done
done

echo "########################################"
echo "########## Scenario 2 - TEST ###########"
echo "########################################"

## Scenario 2: For all kernels, for M, N, K ∈ [256, 512], with best block dims
